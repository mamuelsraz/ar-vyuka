...
public class SimpleHelvetica : MonoBehaviour
{
    ...
    public void GenerateText()
    {
        ...
        for (int ctr = 0; ctr <= Text.Length - 1; ctr++)
        {
            ...
            if (childObjectName != " ")
            {
                ...
                {
                    if (transform.Find("_Alphabets/" + childObjectName) != null)
                    {
                        LetterToShow = transform.Find("_Alphabets/" + childObjectName).gameObject;
                    }
                    else { LetterToShow = transform.Find("_Alphabets/_").gameObject; }
                }
        ...
        transform.localPosition = new Vector3(-(longestPos.x / 2) * transform.localScale.x, -(longestPos.y) * transform.localScale.y, 0);
    }
    ...
    void AddLetter(GameObject LetterObject)
    {
        ...
        if (CharXLocation > longestPos.x) longestPos.x = CharXLocation;
        if (CharYLocation < longestPos.y) longestPos.y = CharYLocation;
